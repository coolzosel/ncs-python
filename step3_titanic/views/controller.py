...
from step3_titanic.models.service import Service
import pandas as pd
...
class Controller(object):
    ...
    def preprocess(self, train, test) -> object:
        ...
        # print(f'전처리 마감 후 칼럼: {this.train.columns}')
        # print(f'train의 널 수량: {this.train.isnull().sum()}')
        # print(f'test의 널 수량: {this.test.isnull().sum()}')
        return this
        '''
        전처리 마감 후 칼럼: Index(['Survived', 'Pclass', 'Sex', 'Embarked', 'Title', 'AgeGroup',
       'FareBand'],
          dtype='object')
