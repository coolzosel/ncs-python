...
from nltk.tokenize import word_tokenize #토크나이저 = 공백기준으로 글자 자르는거
import nltk
import re
import pandas as pd
...
class SamsungReport(object):
    ...
    def remove_stopword(self):
        ...
        texts = [text for text in tokens if text not in stopwords] # not in = stopwords에 없는 단어만 뽑아내기
        return texts
    ...
    def draw_wordcloud(self):
        texts = self.remove_stopword()
        wcloud = WordCloud('./data/D2Coding.ttf', relative_scaling=0.2, # 폰트 지정, 이미지사이즈
